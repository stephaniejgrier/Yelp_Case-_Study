...
#-------------------------------------------------------------------------------
setwd("C:/Users/sgrie/Documents/Yelp_Case _Study/Yelp_Data")
#-------------------------------------------------------------------------------
...
# Show many restaurants you found
...
base_path <- "C:/Users/sgrie/Documents/Yelp_Case _Study/Yelp_Data/"
...
for (metro in names(metro_files)) {
  file <- file.path(base_path, paste0(metro_files[metro], ".csv"))
...
viz_data <- final_data_cleaned %>%
  ...
  ungroup()
write.csv(viz_data,"C:/Users/sgrie/Documents/Yelp_Case _Study/Yelp_Data/viz_data.csv", row.names = FALSE)
#-------------------------------------------------------------------------------
...
write_csv(metro_areas_for_map, "metro_areas_for_map.csv")

# You now have columns:
...
# (optional) attach population if you have a ZCTA table
if (exists("all_zcta_population_data")) {
  ...
  # fallback: use whatever population you have in final_data_cleaned
  zip_base <- zip_base %>%
    left_join(
      f %>%
        group_by(zip_code) %>%
...
ideal_graph_data<-read.csv("C:/Users/sgrie/Downloads/Sheet 9_data.csv")
# ---- Packages ----
library(readr)
library(dplyr)
...
# ---- Load exported CSV ----
df_raw <- read_csv("Sheet 9_data.csv", show_col_types = FALSE) |>
  clean_names()
...
left_top <- max(df$rpq, na.rm = TRUE)  # we’ll let the left axis top equal the max RPQ
...
ggplot(df, aes(x = city, group = 1)) +
  ...
  scale_fill_manual(
    ...
    guide = "none"   # hide fill legend; keep color legend if you want
  ) +
  scale_color_manual(
    name   = NULL,
    values = c("Residents/QSR" = col_res, "Customer satisfaction" = col_csat)
